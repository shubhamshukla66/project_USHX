...
const WoymingLlcProfile = require('../models/woymingLlc')
...
router.post('/seller/register', function (request, response) {
  const { fullName, email, password, origin, referredBy, role, roleId, consent, device_token } = request.body
...
router.post('/woymingllc/register', function (request, response) {
  const { llcName, alternateName, email, fullName, birthdate, phone, address, city, state, zipcode, device_token } = request.body
  var { origin } = request.body;
  if (!llcName) response.send(utils.createError("LLC Name Required!"))
  if (!email) response.send(utils.createError("Email Required!"))
  if (!utils.validateEmail(email.toLowerCase())) {
    response.send(utils.createError("Invalid email"));
  } else {
    let woymingLlcProfile = new WoymingLlcProfile();
    if (origin) woymingLlcProfile.origin = origin;
    if (llcName) woymingLlcProfile.llcName = llcName;
    if (alternateName) woymingLlcProfile.alternateName = alternateName;
    woymingLlcProfile.fullName = fullName;
    if (birthdate) woymingLlcProfile.birthdate = birthdate;
    if (phone) woymingLlcProfile.phone = phone;
    if (address) woymingLlcProfile.address = address;
    if (city) woymingLlcProfile.city = city;
    if (state) woymingLlcProfile.state = state;
    if (zipcode) woymingLlcProfile.zipcode = zipcode;
    woymingLlcProfile.email = email;
    woymingLlcProfile.save(async (error, woymingLlcProfile) => {
      console.log(error, woymingLlcProfile);
      if (error) {
        return response.send(utils.createResult("Database error"))
      } else {
        WyomingEmailSend(woymingLlcProfile, response, error);
      }
    })
  }
})
...
router.get('/getAll/:type?', function (request, response) {
  ...
  if (type) search = { status: type, deleted: false, $or: [{ isProfileComplete: true }, { role: 'affiliate' }, { role: 'investor' }] }
  User.find(search)
    // .populate({path:'role'})
    .sort({ createdAt: -1 })
    .exec((error, users) => {
...
router.post('/userImage', upload.single('image'), async (request, response) => {
  await s3ImageUpload(request.file, 'uploads/users');
          ...
          if (request.files) {
            ...
            if (files) {
              ...
              for (let i = 0; i < files.length; i++) {
                let file = files[i];
                await s3ImageUpload(file, 'uploads/users');
                // console.log("extracted", file);
...
router.get('/application/:userId', function (request, response) {
  ...
  User.findOne({ _id: userId, deleted: false }, (error, user) => {
    ...
    if (error) {
      ...
      let application = {};
      InvestroProfile.findOne({ userId }, (err, profile) => {
        console.log({ profile });
        if (err) {
          response.send(utils.createError('Database error'));
        } else if (!profile) {
...
router.post('/seller/stepThree/:userId?', uploadSellers.single('appraisalFile'), async (request, response) => {
 ...
 */
router.post('/contractor/stepOne', uploadContractors.single('governmentId'), async (request, response) => {
  await s3ImageUpload(request.file, 'uploads/contractors');
...
router.post('/contractor/stepTwo', uploadContractors.single('tradeLiscence'), async (request, response) => {
  await s3ImageUpload(request.file, 'uploads/contractors');
    ...
    Property.findOne({ _id: propertyId }, (err, property) => {
      ...
      property.save(async (err, property) => {
        ...
        mailer.createNotification(property._id, title, message, 'admin', 'property', (err, data) => {
          mailer.createNotification(property.seller_id, title, message, 'seller', 'property', (err, data) => {
            console.log("createNotification", err, data);
...
/* To be used when creating an account of Wyoming LLC send email to info and user */
async function WyomingEmailSend(woymingLlcProfile, response, error) {
  let title = `Account Created`;
  message = `<h3>Hello ${woymingLlcProfile.fullName}!</h3><p>Your account has been created on the US Housing Exchange.</p><p>US Housing Exchange</p>`

  let emailTemplate = paymentHtmlTemplate();

  // mailer.sendEmail('njain923@gmail.com', 'Wyoming LLC Account Created', emailTemplate, (err, data) => {
  //   console.log("User Email", err, data);
  //   response.send(utils.createResult(error, {}))
  // })

  mailer.sendEmail(woymingLlcProfile.email, title, emailTemplate, (err, data) => {
    let messageHtml = `<h3>Hello Wyoming LLC Account Created by ${woymingLlcProfile.fullName}!</h3>
    <p>Desired LLC name: ${woymingLlcProfile.llcName}</p>
    <p>Alternate name: ${woymingLlcProfile.alternateName}</p>
    <p>Full Name: ${woymingLlcProfile.fullName}</p>
    <p>Date of Birth: ${woymingLlcProfile.birthdate}</p>
    <p>Phone number: ${woymingLlcProfile.phone}</p>
    <p>Address: ${woymingLlcProfile.address}</p>
    <p>State: ${woymingLlcProfile.state}</p>
    <p>City: ${woymingLlcProfile.city}</p>
    <p>Zip Code: ${woymingLlcProfile.zipcode}</p>
    <p>Email: ${woymingLlcProfile.email}</p>
    <p>US Housing Exchange</p>`
    mailer.sendEmail('info@ushousingexchange.com', 'Wyoming LLC Account Created', messageHtml, (err, data) => {
      console.log("User Email", err, data);
      response.send(utils.createResult(error, {}))
    })
  })
...
<head>
  <meta charset="utf-8">
  <!--====== Title ======-->
  <title>USHx Email Template</title>
                                              ...
                                              <td class="td"
                                                  ...
                                                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                      <tr>
                                                          <td class="fluid-img"
                                                              style="font-size:0pt; line-height:0pt; text-align:left;">
                                                              <img src="https://s3.ap-south-1.amazonaws.com/sanswebsolutions.com/img/banner.png"
                                                                  width="650" height="300" border="0"
                                                                  alt="banner img" />
                                                          </td>
                                                      </tr>
                                                  </table>
                                              ...
                                              <td>
                                                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                      <tr>
                                                          <br />
                                                          <td class=" centered"
                                                              style="padding:10px 0; color:#ffffff; font-family:'Roboto', sans-serif; font-size:20px; margin-top: 20px; line-height:27px; text-align:center;"
                                                              bgcolor="#0077FF">Please send $249 through one of the
                                                              following apps</td>
                                                      </tr>
                                                  </table>
                                              </td>
                                                  ...
                                                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                      <tr>
                                                          ...
                                                          <td class=" centered"
                                                              style="padding:10px 0; color:#ffffff; font-family:'Roboto', sans-serif; font-size:20px; margin-top: 20px; line-height:27px; text-align:center;">
                                                              <img class="mx-2"
                                                                  src="https://s3.ap-south-1.amazonaws.com/sanswebsolutions.com/img/onepaymentLogo.png"
                                                                  width="80" height="50" alt="banner img" />
                                                              <img class="mx-2"
                                                                  src="https://s3.ap-south-1.amazonaws.com/sanswebsolutions.com/img/twopaymentLogo.png"
                                                                  width="80" height="50" alt="banner img" />
                                                              <img class="mx-2"
                                                                  src="https://s3.ap-south-1.amazonaws.com/sanswebsolutions.com/img/threepaymentLogo.jpeg"
                                                                  width="80" height="50" alt="banner img" />
                                                              <img class="mx-2"
                                                                  src="https://s3.ap-south-1.amazonaws.com/sanswebsolutions.com/img/fourpaymentLogo.jpeg"
                                                                  width="80" height="50" alt="banner img" />
                                                          </td>
                                                      </tr>
                                                  </table>
                                                  ...
                                                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                      <tr>
                                                          ...
                                                          <td class=" centered"
                                                              style="padding:10px 0; color:#ffffff; font-family:'Roboto', sans-serif; font-size:20px; margin-top: 20px; line-height:27px; text-align:center;">
                                                              <a target="_blank"
                                                                  href="https://ushousingexchange.com/assets/llcpayment.html">
                                                                  <span
                                                                      style=" background-color:  #08c !important; padding: 10px; color: white;">
                                                                      Pay Now
                                                                  </span>
                                                              </a>
                                                          </td>
                                              ...
                                              <td>
                                                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                      <tr>
                                                          <br />
                                                          <td class=" centered"
                                                              style="padding:10px 0;  font-family:'Roboto', sans-serif; font-size:20px; margin-top: 20px; line-height:27px; text-align:center;">
                                              ...
                                              <td>
                                                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                      <tr>
                                                          <br />
                                                          <td class=" centered"
                                                              style="color:#ffffff; font-family:'Roboto', sans-serif;  text-align:center;">
                                                              <img src="https://s3.ap-south-1.amazonaws.com/sanswebsolutions.com/img/Halo.png"
                                              ...
                                              <td>
                                                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                      <tr>
                                                          ...
                                                          <td class=" centered"
                                                              style="padding:10px 0;  font-family:'Roboto', sans-serif; font-size:20px; margin-top: 20px; line-height:27px; text-align:center;">
                                                              A Non-Profit partner of the <br />
                                                              &nbsp;US Housing Exchange
                                                          </td>
                                                      </tr>
                                                  </table>
